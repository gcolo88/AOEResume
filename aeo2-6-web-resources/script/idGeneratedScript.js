...
function goBackOnRollOff(mso_id) {
var mso_element = document.getElementById(mso_id);
if(mso_element) {
removeClass(mso_element, '_idGenStateHide')
var mso_states = mso_element.getElementsByTagName('div');
for (var i = 0, state; state = mso_states[i]; i++) {
removeClass(state, '_idGenCurrentState');
var cn = state.className;
if (cn.indexOf('_idGenPreviousState') != -1 ) {
removeClass(state, '_idGenPreviousState');
removeClass(state, '_idGenStateHide');
addClass(state, '_idGenCurrentState');
}
}
}
}
